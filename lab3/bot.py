...
def init_db():
    ...
    # Таблица уже обработанных продаж (чтобы не дублировать уведомления)
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS seen_sold (
...
def check_sold():
    ...
    for item in items:
        item_id = item["id"]
        # Проверяем, видели ли мы этот предмет раньше
        cursor.execute('SELECT 1 FROM seen_sold WHERE item_id = ?', (item_id,))
        if cursor.fetchone():
            continue

        # Новая продажа — записываем
        cursor.execute('INSERT INTO seen_sold (item_id) VALUES (?)', (item_id,))
...
async def cmd_inventory(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/inventory — полный обзор инвентаря (новые + на продаже + проданные)"""
    try:
        new_items = get_inventory(statuses=0)
        on_sale = get_inventory(statuses=1)
        sold = get_inventory(statuses=2)

        on_sale_total = sum(i.get("price", 0) for i in on_sale) / 100

        # Считаем уникальные предметы на продаже
        items_count = {}
        for item in on_sale:
            name = item["hash_name"]
        ...
        lines = [
            f"🎒 Инвентарь\n{'─' * 26}",
            f"📥 Новых (не выставлены): {len(new_items)}",
            f"🏷 На продаже: {len(on_sale)} шт. на {on_sale_total:.2f} ₽",
            f"✅ Продано: {len(sold)}",
            f"\n🏷 На продаже по предметам:"
        ]

        # Сортируем по количеству
        sorted_items = sorted(items_count.items(), key=lambda x: x[1], reverse=True)
        for name, cnt in sorted_items[:25]:
            price_info = ""
            for item in on_sale:
                if item["hash_name"] == name:
                    price_info = f" — {item.get('price', 0) / 100:.2f} ₽"
                    break
            suffix = f" x{cnt}" if cnt > 1 else ""
            lines.append(f"  • {name}{suffix}{price_info}")

        if len(sorted_items) > 25:
            lines.append(f"\n  ...и ещё {len(sorted_items) - 25} наименований")
